...
from multi_agent_video.local_llm import LocalLLM
...
class SharedCapabilitiesService:
    """Cross-system capability adapters with stable, structured contracts.

    This default implementation is local-only and intentionally lightweight.
    It provides the four integration contracts planned for cross-product reuse.
    """
    ...
    def run_local_llm_with_fallback(
        ...
        if self._llm is not None:
            try:
                text = self._llm.chat(system_prompt=system_prompt, user_prompt=user_prompt)
                if text.strip():
                    return LLMFallbackResult(text=text.strip(), provider_used="local_llm", errors=[])
